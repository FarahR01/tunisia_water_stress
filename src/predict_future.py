...
def prepare_features_for_prediction(
    feature_data: pd.DataFrame, feature_columns: list, target_year: int = None
) -> pd.DataFrame:
    """
...
def predict_water_stress_2030(
    model_path: str,
    processed_data: pd.DataFrame,
    feature_columns: list,
    last_year: int = 2024,
    target_year: int = 2030,
    scenario_method: str = "linear",
    ...
    scenario = create_2030_scenario(
        processed_data, last_year=last_year, target_year=target_year, trend_method=scenario_method
    )

    # Get features for 2030 (including year if requested)
    features_2030 = prepare_features_for_prediction(
        scenario, feature_columns, target_year=target_year
    )
