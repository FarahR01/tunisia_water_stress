...
def plot_top_features(models_dict, feature_names, X_test, top_n=10, out_dir='models'):
    ...
    if len(importance_dfs) > 1:
        ...
        top_features = comparison_df.groupby('feature')['importance'].mean().nlargest(top_n).index.tolist()
        ...
        for model_name in filtered['model'].unique():
            model_data = filtered[filtered['model'] == model_name].sort_values('importance', ascending=True)
            plt.barh(model_data['feature'].values, model_data['importance'].values, label=model_name, alpha=0.7)
