...
class Settings(BaseSettings):
    ...
    # API Settings
    api_title: str = Field(
        default="Tunisia Water Stress ML API", description="API title for OpenAPI docs"
    )
    api_version: str = Field(default="1.0.0", description="API version string")
    api_debug: bool = Field(default=False, description="Enable debug mode (shows stack traces)")
    api_prefix: str = Field(default="/api/v1", description="API route prefix for versioning")

    # CORS Settings
    cors_origins: list[str] = Field(
        default=["*"], description="Allowed CORS origins. Use specific origins in production."
    )
    ...
    models_dir: Path = Field(
        default=PROJECT_ROOT / "models_tuned", description="Directory containing trained models"
    )
    default_model_name: str = Field(
        default="Lasso", description="Default model to use for predictions"
    )
