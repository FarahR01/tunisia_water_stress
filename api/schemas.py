...
class ModelsInfoResponse(ResponseBaseModel):
    ...
    default_model: str = Field(
        ...,
        description="Default model used when none specified"
    )
    metrics: list[ModelInfo] = Field(
        ...,
        description="Performance metrics for each model"
    )
    feature_columns: list[str] = Field(
        ...,
        description="Feature columns expected by the models"
    )
...
class PredictionOutput(ResponseBaseModel):
    ...
    predicted_water_stress: float = Field(
        ...,
        description="Predicted water stress level (%)"
    )
    model_used: str = Field(
        ...,
        description="Name of model used for prediction"
    )
...
class ScenarioOutput(ResponseBaseModel):
    ...
    target_year: int = Field(
        ...,
        description="Target year of scenario"
    )
    predicted_water_stress: float = Field(
        ...,
        description="Predicted water stress level (%)"
    ...
    trend_method: str = Field(
        ...,
        description="Trend extrapolation method used"
    )
...
class YearRangeParams(StrictBaseModel):
    ...
    start_year: int = Field(
        ...
        ge=1960,
        le=2100,
        description="Start year of range"
    )
    end_year: int = Field(
        ...,
        ge=1960,
        le=2100,
        description="End year of range"
    )
    model_name: str | None = Field(
        None,
