...
system_router = APIRouter(prefix="", tags=["System"], responses={500: {"model": ErrorResponse}})
...
models_router = APIRouter(
    prefix="/models", tags=["Models"], responses={500: {"model": ErrorResponse}}
)
...
def get_models_info(service: ModelServiceDep) -> ModelsInfoResponse:
    ...
    model_metrics = [
        ModelInfo(
            name=model_name, mae=round(m["MAE"], 4), rmse=round(m["RMSE"], 4), r2=round(m["R2"], 4)
        )
...
def predict_water_stress(input_data: PredictionInput, service: ModelServiceDep) -> PredictionOutput:
    """
...
def batch_predict(
    input_data: BatchPredictionInput, service: ModelServiceDep
) -> BatchPredictionOutput:
    """
...
def predict_year_range(
    ...
    for year in range(start_year, end_year + 1):
        prediction, model_used, features = service.predict(year=year, model_name=model_name)
        results.append(
            PredictionOutput(
                year=year,
                predicted_water_stress=round(prediction, 2),
...
def predict_scenario(input_data: ScenarioInput, service: ModelServiceDep) -> ScenarioOutput:
    """
    Generate a scenario-based prediction for a future year.
...
def get_v1_routers() -> list[APIRouter]:
    ...
    return [system_router, models_router, predictions_router, scenarios_router]
