...
from .config import FEATURE_COLUMNS, TARGET_COLUMN, VALID_TREND_METHODS, get_settings
...
class ModelService:
    ...
    def __init__(self, models_dir: Path | None = None, data_path: Path | None = None):
        """
    ...
    def _load_models(self) -> None:
        ...
        if not self.models:
            raise ModelLoadError(str(self.models_dir), "No models found or all failed to load")
    ...
    def _fit_scaler(self) -> None:
        ...
        try:
            available_features = [
                c for c in FEATURE_COLUMNS if c in self.historical_data.columns or c == "year"
            ]
        ...
        for key, col in feature_mapping.items():
            ...
            if method == "linear":
                ...
                if values[-recent_years] > 0:
                    ...
                    extrapolated[key] = float(
                        values[-1] * (growth_rate ** (target_year - last_year))
                    )
                else:
                    extrapolated[key] = float(values[-1])
    ...
    def predict_scenario(
        ...
        prediction, model_used, features = self.predict(
            ...
            freshwater_withdrawals_agriculture=extrapolated.get(
                "freshwater_withdrawals_agriculture"
            ),
