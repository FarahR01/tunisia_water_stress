    ...
    if _model_service_instance is None:
        try:
            _model_service_instance = ModelService(
                models_dir=settings.models_dir, data_path=settings.processed_data_path
            )
        except Exception as e:
            raise ServiceUnavailableError(detail=f"Failed to initialize model service: {str(e)}")
...
def rate_limit_check(request: Request, settings: SettingsDep) -> None:
    """
